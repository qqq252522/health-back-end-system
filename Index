...
<div class="navbar">
  ...
  <a href="index.php">處方來源排行(院所)</a>
	...
	<h1 id = "header">來源排行</h1>
	<div class="tab" style="z-index: -1; position: absolute;"></div>
	<div class="tab" id="tab">
		<button class="tablinks" onclick="window.location.href='index.php'">上一層</button>
		<button class="tablinks" onclick="selection=1;tableText(icd,time_1,time_2,selection)">ICD排行</button>
		<button class="tablinks" onclick="selection=2;tableText(icd,time_1,time_2,selection)">產品排行</button>
		<button class="tablinks" onclick="selection=3;tableText(icd,time_1,time_2,selection)">廠商排行</button>
	</div>
	<div class="dd-group">
		<div class="dropdown">
	...
	<table class="data-table" id = "tableID">
		<thead>
			<tr>
				<th onclick="sortTable(0)">醫院名稱 ⇅</th>
	...
	<script type="text/javascript">
			...
			if (table != null) {
				for (var i = 1; i < table.rows.length; i++) {
		           ...
		           table.rows[i].cells[0].onclick = function () {//這可以偵測點哪格 並把那格的數值傳給tableText函式
						...
						icd=this.innerHTML;
						document.getElementById("header").innerHTML="醫院名稱:"+icd;
		   ...
		   function tableText(value,time1,time2,select) {//value為icd,select為選項
		    $_xmlHttpRequest();
		    xmlHTTP.open("GET","index_2.php?icd="+value+"&time1="+time1+"&time2="+time2+"&select_op="+select,true);
	        ...
	        xmlHTTP.onreadystatechange=function check_user()
	        {
	            if(xmlHTTP.readyState == 4)
	            {
	                if(xmlHTTP.status == 200)
	                {
	                    ...
	                    console.log(time1-100,time2-100);
	                    if(select==3){
	                    	tableBar(1);
	                    	tableBar(2);
	                    	tableBar(3);
							...
							if (table != null) {
								for (var i = 1; i < table.rows.length; i++) {
						           ...
						           table.rows[i].cells[0].onclick = function () {//這可以偵測點哪格 並把那格的數值傳給tableText函式
						           	   selection=1;
						               tableText(this.innerHTML,time1,time2,selection);
						               var icd_ch = this.parentNode.cells[1].innerHTML;//把ICD的中文標題儲存給變數 讓下一個頁面可以用到
						               icd=this.innerHTML;
						               document.getElementById("header").innerHTML="醫院名稱:"+icd;
						               //顯示類似於 ICD:I10 本態性(原發性)高血壓排行
