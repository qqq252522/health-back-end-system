...
<div class="navbar">
  ...
  <a href="index.php">處方來源排行(院所)</a>
	...
	<h1 id = "header">ATC 排行</h1>
	<div class="tab" style="z-index: -1; position: absolute;"></div>
	<div class="tab" id="tab">
		<button class="tablinks" onclick="window.location.href='atc.php'">上一層</button>
		<button class="tablinks" onclick="selection=1;tableText(icd,time_1,time_2,selection)">來源排行</button>
	...
	<table class="data-table" id = "tableID">
		...
		<tfoot>
			<tr>
				<th colspan="3">排行前<?=number_format($no-1)?> TOTAL</th>
	...
	<script type="text/javascript">
			...
			if (table != null) {
				for (var i = 1; i < table.rows.length; i++) {
		           ...
		           table.rows[i].cells[0].onclick = function () {//這可以偵測點哪格 並把那格的數值傳給tableText函式
					...
					icd=this.innerHTML;
					document.getElementById("header").innerHTML="ATC:"+icd+"  "+icd_ch+"排行";
		   ...
		   function tableText(value,time1,time2,select) {//value為icd,select為選項
		    $_xmlHttpRequest();
		    xmlHTTP.open("GET","atc_2.php?icd="+value+"&time1="+time1+"&time2="+time2+"&select_op="+select,true);
